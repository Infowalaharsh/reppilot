...
import { isNativeApp, nativeGoogleEnabled, signInWithNativeGoogle } from "@/lib/native-auth";
    ...
    try {
      if (mode === "signup") {
        const { data, error } = await supabase.auth.signUp({
          email,
          password,
          options: isNativeApp ? undefined : { emailRedirectTo: window.location.origin },
        });
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-6 py-10">
      <div className="mx-auto w-full max-w-sm">
        ...
        {(!isNativeApp || nativeGoogleEnabled) && (
          <>
            <button
              onClick={google}
              disabled={busy}
